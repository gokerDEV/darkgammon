...
- Apple Login
- Profil oluşturma
- Profil sayfası `/ni/@[handler]`
- Profile özel QR code (oyun daveti)
...
Kullanıcı login ama profili yoksa ve  invite link ile gelirse
  -> /profile/nickname (sadece  nickname belirler ve oyuna başlar diger bilgiler random doldurulur)
...
```txt
/ni/@[handle] değişebilir.
/ni/![token] kalıcı kalır.
```
Kullanici yeni QR  uretebilmeli. Yeni QR  token  değiştirilmeli. 

Önerilen QR target:

```txt
https://tavla.be/ni/![qrToken]
```
...
- `/ni/@[handle]` public olarak açılır.
- `/ni/![token]` profile resolve eder.
...
9. B kabul ya da red notification'i alir. 
10. B Red alirsa ayni kisiye 1dk boyunca ikinci istek  gonderemez. Gun icinde  ayni kisiye en fazla 3  istek gonderilebilir.
11. Kabul edilirse game session oluşturulur.
12. A ve B /ni/[sessionId] sayfasına yönlendirilir.
```
...
Video export ürünün en güçlü parçası. Her kullanici icin  son 10 oyunun oyun kaydi  (JSON hamleler) mongoDB uzerinde tutulmali. Bu veri ile clientside  replay ve video generation yapilabilmeli.  KUllanicinin profilde  son 10 oyundaki basarisi  '8/10' gibi gorunmeli ve  oyun listesi olmali  ve tiklayinca  onun  replayini  izleyebilmeli. 
