...
import type {
  ...
  Color,
  Move,
} from "@/lib/games/backgammon/types";
...
function ReplayContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/replay/${sessionId}`)
      ...
      .then((data) => {
        ...
        if (data.snapshots) setSnaps(data.snapshots);
      })
      .catch(console.error);
    return () => {
      mounted = false;
  ...
  const winnerName =
    winnerColor === "white"
      ? record.metadata.whitePlayer
      : record.metadata.blackPlayer;
  const winnerMessage =
    winnerColor === "white"
      ? record.metadata.whiteMessage
      : record.metadata.blackMessage;
  const winnerGif =
    winnerColor === "white"
      ? record.metadata.whiteGif
      : record.metadata.blackGif;
  const loserName =
    winnerColor === "white"
      ? record.metadata.blackPlayer
      : record.metadata.whitePlayer;
  ...
  const totalEvents = record.isReal
        ...
        if (record.isReal) {
          ...
          setState((prev) => {
            if (ev.type === "roll") {
              ...
              const mv: Move = ev.move as Move;
              return applyMove(prev, ev.color as Color, mv);
  ...
  async function handleExport() {
    ...
    try {
      ...
      let timestampUs = 0;
      const delayUs = Math.floor(
        (targetDuration * 1000 * 1000) / Math.max(1, totalEvents),
      );
      ...
      <div className="flex-1 flex flex-col pt-0 relative overflow-hidden bg-background">
        ...
        >
          <GameHeader
            ...
            whiteAvatarUrl={record.metadata.whiteAvatarUrl}
            whiteStatus="connected"
            blackPlayer={record.metadata.blackPlayer}
            blackAvatarUrl={record.metadata.blackAvatarUrl}
          ...
          {phase === "outro" && (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center p-6 bg-background/90 backdrop-blur-sm text-foreground gap-6">
              ...
              />
              <p className="text-xl font-medium italic text-center px-4">
                "{winnerMessage}"
              </p>
              {!isExporting && (
                <Button
