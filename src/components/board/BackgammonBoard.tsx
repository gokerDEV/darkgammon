...
const pointX = [
  6.24, 13.67, 21.09, 28.54, 35.96, 43.39, 56.45, 63.91, 71.33, 78.76, 86.2,
  93.63,
];
...
export function BackgammonBoard({
  ...
  const moves = useMemo(
    () => (myColor ? legalMoves(state, myColor) : []),
    [state, myColor],
  );
  ...
  const canBearOff =
    isMyTurn &&
    selected !== null &&
    moves.some((m) => m.from === selected && m.to === "off");
  ...
  function handleEmptyClick(e: React.MouseEvent) {
    ...
    const isCheat = process.env.CHEATING === "true" && e.metaKey;
  ...
  function doubleClickPoint(idx: number) {
    if (!isMyTurn || !myColor) return;
    const cand = moves.filter((m) => m.from === idx && m.to === "off");
    if (cand.length === 0) return;
    cand.sort((a, b) => a.die - b.die);
    onApply(cand[0]);
    onSelect(null);
  ...
  function renderPoint(idx: number, top: boolean, i: number) {
    ...
    return (
      <button
        ...
        onDoubleClick={() => doubleClickPoint(idx)}
        style={{
          left: `${x}%`,
          [top ? "top" : "bottom"]: "10.4%",
          width: "6.5%",
      ...
      >
        <div className="flex flex-col items-center gap-[1px] w-full mt-1 mb-1 relative h-full">
          {Array.from({ length: Math.min(point.count, 5) }).map(
            (_, idxInner) => (
              <div
                // biome-ignore lint/suspicious/noArrayIndexKey: visual
                key={`point-${idxInner}`}
                className={`absolute w-full flex justify-center ${top ? "top-0" : "bottom-0"}`}
                style={{ [top ? "top" : "bottom"]: `${idxInner * 20}%` }}
              >
          ...
          {point.count > 5 && (
            <span
              className={`absolute z-10 ${top ? "top-full mt-1" : "bottom-full mb-1"} left-1/2 -translate-x-1/2 rounded-full bg-[#1c1c1e]/85 px-1.5 py-0.5 text-[10px] font-bold text-white shadow-sm`}
            >
              {point.count}
            </span>
          )}
        ...
        {isLegal && (
          <span className="absolute left-1/2 top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#0a84ff] shadow-[0_0_0_7px_rgba(10,132,255,0.18)]" />
        )}
      </button>
    );
  }
      ...
      <div className="absolute inset-0 z-10 pointer-events-none">
        ...
        <div className="absolute top-[10.4%] bottom-[10.4%] left-[50%] w-12 -translate-x-1/2 flex flex-col justify-between py-4 pointer-events-none">
          <BarStack
            ...
            count={isBlack ? state.bar.black : state.bar.white}
          ...
          <BarStack
            ...
            count={isBlack ? state.bar.white : state.bar.black}
            selected={false}
            onClick={clickBar}
          />
        </div>
        ...
        <div className="absolute top-0 left-0 w-[33.6%] h-[6.75%] flex items-center justify-center px-4 pointer-events-auto">
          <div className="flex w-full items-center justify-center h-full">
            <OffTray
              color={isBlack ? "white" : "black"}
              count={isBlack ? state.off.white : state.off.black}
            />
          </div>
        </div>
        ...
        <div className="absolute bottom-0 right-0 w-[33.6%] h-[6.75%] flex items-center justify-center px-4 pointer-events-auto">
          ...
          >
            <OffTray
              color={isBlack ? "black" : "white"}
              count={isBlack ? state.off.black : state.off.white}
            />
          </button>
        </div>
        ...
        <div className="absolute bottom-0 left-0 w-[33.6%] h-[6.75%] flex items-center justify-start px-4 pointer-events-auto">
          <button
            type="button"
...
function Checker({ color }: { color: Color | null }) {
  ...
  return (
    <svg
      className="w-[110%] shrink-0 drop-shadow-[0_3px_5px_rgba(28,28,30,0.3)]"
      viewBox="0 0 35 35"
      aria-hidden="true"
    >
...
function _Die({ v }: { v: number }) {
  return (
    <svg
      role="img"
      className="h-14 w-14 shrink-0 drop-shadow-[0_4px_8px_rgba(28,28,30,0.2)]"
      viewBox="0 0 100 100"
      aria-label={`Die ${v}`}
    >
...
function _DoublingCube({ value }: { value: number }) {
  return (
    <svg
      className="h-8 w-8 shrink-0 drop-shadow-[0_4px_8px_rgba(28,28,30,0.2)]"
      viewBox="0 0 100 100"
      aria-hidden="true"
    >
    ...
    >
      {count > 0 ? (
        <>
          ...
          <span className="text-[11px] font-bold text-[#1c1c1e] bg-white/60 px-1.5 rounded-full">
            {count}
          </span>
        </>
      ) : null}
