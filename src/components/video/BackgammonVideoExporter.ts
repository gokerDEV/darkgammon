...
import type { BgSession, BgState } from "@/lib/games/backgammon/types";
...
function drawBoard(ctx: CanvasRenderingContext2D, state: BgState) {
  ...
  ctx.textBaseline = "top";
  ctx.fillText(
    `Off W: ${state.off.white}   Off B: ${state.off.black}`,
    x0,
    y0 + boardH + 16,
  );
  ctx.textAlign = "right";
...
export async function exportBackgammonVideo(
  ...
  const snaps =
    opts.snapshots.length > 0 ? opts.snapshots : [opts.session.state];
  const PER_SNAP = Math.max(
    220,
    Math.min(600, Math.floor(10_000 / snaps.length)),
  );
  ...
  await new Promise<void>((resolve) => {
    const loop = () => {
      ...
      if (t > SCENE_OPEN) {
        idx = Math.min(
          snaps.length - 1,
          Math.floor((t - SCENE_OPEN) / PER_SNAP),
        );
      }
