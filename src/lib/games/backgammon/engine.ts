...
export function startingState(): BgState {
  const points: Point[] = Array.from({ length: 24 }, () => ({
    color: null,
    count: 0,
  }));
...
export function legalMoves(state: BgState, color: Color): Move[] {
  ...
  if (barCount(state, color) > 0) {
    for (const d of dice) {
      ...
      if (canLand(state, dest, color))
        moves.push({ from: "bar", to: dest, die: d });
    }
    return moves;
  }
...
export function acceptDouble(state: BgState): BgState {
  ...
  return {
    ...state,
    cube: { value: state.cube.value * 2, owner },
